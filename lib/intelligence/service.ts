...
async function buildInput(symbol: string): Promise<NormalizedSymbolInput> {
  try {
    ...
    return normalizeSymbolInput({
      ...
      providerLatencyMs: live.providerLatencyMs ?? null,
      quoteRetryCount: live.retryCount ?? 0,
      staleAgeSeconds: live.staleAgeSeconds ?? null,
      quoteQuotaStatus: live.quotaStatus ?? null,
      support,
      resistance,
      lr50: price,
      lr100: price,
