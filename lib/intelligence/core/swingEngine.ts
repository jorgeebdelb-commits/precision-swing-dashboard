...
export function runSwingEngine(input: NormalizedSymbolInput): SwingEngineOutput {
  const hasMarketPrice = input.marketDataState !== "offline" && typeof input.price === "number" && input.price > 0;
  const trendUp = hasMarketPrice && input.lr50 > input.lr100 && input.price >= input.vwap;
  const momentumStrong = input.rsi >= 58 && input.volumeRatio >= 1.2;
  const breakoutQuality = !hasMarketPrice ? "None" : input.price > input.resistance && input.volumeRatio >= 1.3 ? "Clean" : input.price > input.resistance ? "Questionable" : input.price < input.support ? "Failed" : "None";
  ...
  const entry = hasMarketPrice ? `${(input.support * 1.01).toFixed(2)} - ${(input.support * 1.03).toFixed(2)}` : "N/A";
  const stop = hasMarketPrice ? `${(input.support * 0.97).toFixed(2)} - ${(input.support * 0.99).toFixed(2)}` : "N/A";
  const t1 = hasMarketPrice ? (input.resistance * 1.02).toFixed(2) : "N/A";
  const t2 = hasMarketPrice ? (input.resistance * 1.06).toFixed(2) : "N/A";
